...
##3SBATCH --qos=user_qos_timeifler
		...
		do
			...
			do
				...
				then
                    ...
                    do
					    # run chains
					    mpirun -n ${NCPUS} --mca btl tcp,self --oversubscribe python ${SCRIPT} ${NSTEPS} -run_name=${RUN} -Iflux=$(( b + subit * 10)) -sini=${d} -hlr=${c} -sigma_int=1 -PA=0 -fiberconf=${a} -EXP_OFFSET=${TNOM} -NPHOT=3 --mpi
					    # reduce chains
