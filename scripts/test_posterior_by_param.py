...
import utils
import priors
from likelihood import _setup_likelihood_test, log_posterior
...
def main(args):
    ...
    outdir = os.path.join(utils.TEST_DIR, 'test_posterior_by_param')
    ...
    for par, par_range in test_pars.items():
        ...
        # Compute best-fit log posterior
        true_loglike = log_posterior(theta_true, datacube, pars)
        ...
        for i, val in enumerate(np.linspace(
            ...
            loglike[i] = log_posterior(theta, datacube, pars)
            par_val[i] = val
    ...
    plt.suptitle('Log Posterior slice for 1 varying param')
    plt.tight_layout()

    outfile = os.path.join(outdir, f'logpost-slices.png')
    plt.savefig(outfile, bbox_inches='tight', dpi=300)
